...
//#include "include/esql/lightgbm_isolated.h"
...
bool AdaptiveLightGBMModel::TrainWithSplits(
    ...
    std::string param_str = GenerateParameters(params);
    ...
    auto it = params.find("num_iterations");
    if (it != params.end()) {
        try {
    ...
    ExtractNativeMetrics(booster_);

    // Calculate comprehensive metrics
    if (!validation_data.Empty()) {
        std::cout << "\n[LightGBM] Calculating validation metrics..." << std::endl;
        CalculateDetailedValidationMetrics(validation_data);
    } else {
        std::cout << "[LightGBM] WARNING: No validation data provided. Using training data for metrics." << std::endl;

        // Create TrainingData structure from train_data
        TrainingData full_data;
        full_data.features = train_data.features;
        full_data.labels = train_data.labels;
        full_data.valid_samples = train_data.size;
        full_data.total_samples = train_data.size;

        // Call appropriate training metrics method based on problem type
        if (schema_.problem_type == "binary_classification") {
            CalculateBinaryTrainingMetrics(full_data.features, full_data.labels);
        } else if (schema_.problem_type == "multiclass") {
            CalculateMulticlassTrainingMetrics(full_data.features, full_data.labels);
        } else if (schema_.problem_type.find("regression") != std::string::npos ||
                   schema_.problem_type == "count_regression") {
            CalculateRegressionTrainingMetrics(full_data.features, full_data.labels);
        } else if (schema_.problem_type == "poisson") {
            CalculatePoissonTrainingMetrics(full_data.features, full_data.labels);
        } else if (schema_.problem_type == "quantile_regression") {
            CalculateQuantileTrainingMetrics(full_data.features, full_data.labels);
        } else if (schema_.algorithm == "HUBER") {
            CalculateHuberTrainingMetrics(full_data.features, full_data.labels);
        } else if (schema_.algorithm == "TWEEDIE") {
            CalculateTweedieTrainingMetrics(full_data.features, full_data.labels);
        } else if (schema_.algorithm == "GAMMA") {
            CalculateGammaTrainingMetrics(full_data.features, full_data.labels);
        } else {
            CalculateFallbackMetrics(full_data.features, full_data.labels);
        }
    }
